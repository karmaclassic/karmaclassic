...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("karmaclassic"))
        return false;
...
{
    // Convert karmaclassic:// to karmaclassic:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("karmaclassic://"))
    {
        uri.replace(0, 12, "karmaclassic:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "KarmaClassic.lnk";
}
...
{
    return GetAutostartDir() / "karmaclassic.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("KarmaClassic-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  karmaclassic-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("KarmaClassic-Qt"));
